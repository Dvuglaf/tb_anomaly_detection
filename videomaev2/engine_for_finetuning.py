...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(
        ...
        if not math.isfinite(loss_value):
            ...
            print("Loss is {}, stopping training".format(loss_value), file=open('output.txt', 'a'))
            sys.exit(1)
    ...
    print("Averaged stats:", metric_logger, file=open('output.txt', 'a'))
    
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}


@torch.no_grad()
def validation_one_epoch(data_loader, model, criterion, device, wandb_object, wandb_name):
    ...
    for batch in metric_logger.log_every(data_loader, 500, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            _outputs = output.cpu().float().numpy()
            _targets = target.cpu().float().numpy()
            true_labels = []
            pred_labels = []
            for i in range(len(_outputs)):
                if np.all(_targets[i] == 0.):
                    true_labels.append(11)
                else:
                    true_labels.append(np.argmax(_targets[i]))
                    
                if np.all(_outputs[i] < 0.):
                    pred_labels.append(11)
                else:
                    pred_labels.append(np.argmax(_outputs[i]))
        ...
        epoch_true.extend(true_labels)
        epoch_pred.extend(pred_labels)

    # gather the stats from all processes
    
    metric_logger.synchronize_between_processes()
    print(f"* Acc@1: {metric_logger.acc1.global_avg:.3f}, Precision: {metric_logger.precision.global_avg:.3f}, Recall: {metric_logger.recall.global_avg:.3f}", end=', ')
    print(f"F1: {metric_logger.f1.global_avg:.3f}, Loss: {metric_logger.loss.global_avg:.3f}")

    print(f"* Acc@1: {metric_logger.acc1.global_avg:.3f}, Precision: {metric_logger.precision.global_avg:.3f}, Recall: {metric_logger.recall.global_avg:.3f}", end=', ', file=open('output.txt', 'a'))
    print(f"F1: {metric_logger.f1.global_avg:.3f}, Loss: {metric_logger.loss.global_avg:.3f}", file=open('output.txt', 'a'))
    ...
    wandb_object.log({f"Conf Matrix/{wandb_name}": cm}, commit=False)

    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}


@torch.no_grad()
def final_test(data_loader, model, criterion, device, file):
    ...
    for batch in metric_logger.log_every(data_loader, 200, header):
        ...
        for i in range(output.size(0)):
            string = "{} {}\n".format(
                str(output.data[i].cpu().detach().numpy().tolist()),
                str(target[i].cpu().detach().numpy().tolist()))
...
@torch.no_grad()
def inference_on_video(data_loader, model, criterion, device, file):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Inference:'
    ...
    print(
        ...
        .format(
            ...
            #top5=metric_logger.acc5, Acc@5 {top5.global_avg:.3f} 
